...
public class MainAgent {
    ...
    public MainAgent(SubAgentConfig config,
    ...
    public synchronized void initialize(ChatModel chatModel) {
        if (agent == null) {
            ...
            // 设置系统提示词
            builder.systemPrompt(org.noear.solon.ai.codecli.core.SystemPrompt.builder()
    ...
    private List<TeamTask> analyzeAndCreateTasks(Prompt prompt) {
        ...
        // 默认任务（如果没有匹配到特定类型）
        if (tasks.isEmpty()) {
            tasks.add(TeamTask.of("处理用户请求")
                    .description(prompt.getText())
                    .type(TeamTask.TaskType.DEVELOPMENT)
                    .priority(7)
                    .build());
        }
